...
function createMainSwiper(key, selector, isLoop, duration = 3000, autoplayState = "playing") {
    ...
    if (swiper) {
        swiper.on("lock unlock loopFix update resize breakpoint", updateLockState);
    }
...
$(function () {
    ...
    if (mainServiceSwiperInstance) {
        ...
        bindActiveSlideFocus(mainServiceSwiperInstance, mainServiceSelector);

        mainServiceSwiperInstance.on("lock unlock resize breakpoint update loopFix", updateMainServiceLock);
    }
    // e:260728
    ...
    // 마튀 재생/일시정지
    $(".main-client__marquee-button").on("click", function () {
        const $button = $(this);
        const $marquee = $button.closest(".main-client__marquee");
        const isPaused = $marquee.toggleClass("is-paused").hasClass("is-paused");

        $button.toggleClass("is-paused", isPaused).attr("aria-label", isPaused ? "마퀴 재생" : "마퀴 정지");
    });
